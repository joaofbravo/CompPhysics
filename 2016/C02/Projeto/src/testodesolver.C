...
int main() {
	...
	// ex 65 (alterado): Linear pendulum with friction

	//t = x[0]; theta = x[1]; omega = x[2]
	//d^2theta/dt^2 = -g/l*theta - b/m*omega
	...
	//lambda = b/m = 2
	//w0^2 = g/l = 4
	//theta(t) = A*exp(-lambda*x/2)*cos(sqrt(w0^2- lambda^2/4)*x + phi0)
	...
	a.AddPad(vvec, 0, 1, "RK4;t (s);#theta (rad)", "AP", clr1, vtf1);
	a.Draw();
	a.Print("execs/odeteste.pdf");
