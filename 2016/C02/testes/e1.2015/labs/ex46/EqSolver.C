...
{	
	...
	if(GeneralIterator(A,fb,eps,x,1000,true))
		return x;
...
{	
	...
	if(GeneralIterator(A,fb,eps,x,1000,false))
		return x;
...
{	
	...
	// 10 iterations with Jacobi
	if(GeneralIterator(A,fb,eps,x,10,false)){
		#ifdef DEBUG
		cout << __PRETTY_FUNCTION__ << endl;
		#endif
...
int EqSolver::Pivoting(FCmatrix& A, int row, Vec& fb)
{
	...
	int maxindex = A.GetColMax(s, row, row);
...
bool EqSolver::GeneralIterator(FCmatrix& A,Vec& fb, double eps, Vec& x, int N, bool method)
{	
	if(LU_done)
	{
		#ifdef DEBUG
	...
	while (!btol && it < N) 
	{
		...
		for (int i=0; i < A.size(); i++) 
		{
			...
			for (int j=0; j < A.size(); j++)
			{
				if (i != j) 
				{
					// method true=Jacobi, false=GaussSeidel
					if(method)
						x[i] -= (A)[i][j]*x_aux[j];
					else
						x[i] -= (A)[i][j]*x[j];
